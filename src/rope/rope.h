...
//
// Analogy: turning clock hands by different amounts — the relative angle
// between hands stays the same, but the absolute direction changes.
//
// Usage:
//   RoPE rope(128, 2048);           // d_head=128, max_seq=2048
//   rope.forward(Q_h, K_h, 0);      // apply to one head's Q/K
// ---------------------------------------------------------------------------
...
public:
  ...
  explicit RoPE(std::size_t d_head, std::size_t max_seq_len = 2048,
  ...
  ///                   (e.g. 0 for prefill, seq_offset for decode).
  ///
...
public:
  ...
  /// Precompute cos / sin tables.
  ///
  /// @param d_head       Dimension per head (must be even). 必须偶数
  /// @param max_seq_len  Maximum sequence length to support.
  /// @param theta_base   Frequency base (Llama: 10000.0).
  ///
  /// @throws std::invalid_argument if d_head is odd
  ...
  ///
  /// @throws std::invalid_argument if shapes are not [*, d_head_]
  // start_pos序列起始偏移， Decode阶段复用KV Cache时传已有序列长度
  // 原地修改Q, K, shape不变， 无新张量返回
  void forward(Tensor &Q, Tensor &K, std::size_t start_pos = 0) const;
...
private:
  std::size_t d_head_;      // 单个注意头的向量维度
  std::size_t max_seq_len_; // 支持的最大序列长度
  std::size_t n_pairs_;     // d_head_ / 2
                            // 推理只读查表， 不能反复调用std::cos, std::sin
                            // 与计算全局表， 一张表多轮复用
  Tensor cos_;              // [max_seq_len_, n_pairs_]
  Tensor sin_;              // [max_seq_len_, n_pairs_]
};
